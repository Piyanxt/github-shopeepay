...
Read Excel
    ...
    [Arguments]    ${excel_type}               
        ...
        ${CountRow}=    Get Row Count    TC4
        FOR    ${index}    IN RANGE    6    ${CountRow} -2
            ${A1}    Read Cell Data By Name    TC4    A${index}         
            Set Global Variable    ${th}              ${A1}   
            ${B1}    Read Cell Data By Name    TC4    B${index}
            Set Global Variable    ${en}              ${B1}
            ${C1}    Read Cell Data By Name    TC4    C${index}
            Set Global Variable    ${th_in_space}     ${C1}
            ${D1}    Read Cell Data By Name    TC4    D${index}
            Set Global Variable    ${th_output_space}  ${D1}
            ${E1}    Read Cell Data By Name    TC4    E${index}
            Set Global Variable    ${en_in_space}      ${E1}
            ${F1}    Read Cell Data By Name    TC4    F${index}
            Set Global Variable    ${en_output_space}   ${F1}
            ${G1}    Read Cell Data By Name    TC4    G${index}
            Set Global Variable    ${expected_result}   ${G1}
           
            Sleep    2s
            Click Element         xpath=/html/body/div/div/div/div[1]/div[2]/div[1]/div/div[2]/div/div/span[5]
...
Contact Us TH
   ...
   Switch Window    MAIN
    ...
    ${th_output_sytem_space}=    Get Text    xpath=/html/body/div/div/div/div[1]/div[3]/div/div/div
...
Contact Us EN
   ...
   Switch Window    MAIN
    ...
    Sleep    2s
    #ดึงข้อมูลข้อความออกมาเพื่อนดูว่าตรงกับที่ส่งเข้าไปหรือไม่
    ${en_output_sytem_space}=    Get Text    xpath=/html/body/div/div/div/div[1]/div[3]/div/div/div
...
Return th string 
    ...
    editFile           C${index}     TC4     ${excel}.${excel_type}     ${th_string_2}


Return th output system 
    [Arguments]    ${index}    ${excel_type}    ${th_output_string2}
    editFile           D${index}     TC4     ${excel}.${excel_type}     ${th_output_string2}


Return en string 
    [Arguments]    ${index}    ${excel_type}    ${en_string_2}
    editFile           E${index}     TC4     ${excel}.${excel_type}     ${en_string_2}


Return en output system 
    [Arguments]    ${index}    ${excel_type}    ${en_output_string2}
    editFile           F${index}     TC4     ${excel}.${excel_type}     ${en_output_string2}
